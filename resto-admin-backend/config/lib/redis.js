...
exports.deleteRefreshTokenByRsid = function (tokenRsid, done) {

  redisClient.del(
    config.redis.sessionPrefix + tokenRsid, function (err, res) {
      if (err) {
        return done(err);
      }
...
exports.deleteRefreshTokens = function (tokenObjects, done) {

  var redisTokens = _.map(tokenObjects, function (tokenObj) {
    return config.redis.sessionPrefix + tokenObj.rsid;
  });

  redisClient.del(redisTokens, function (err, res) {
    if (err) {
      return done(err);
    }
